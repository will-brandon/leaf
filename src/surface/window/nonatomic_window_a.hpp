/**
 * @file    nonatomic_window_a.hpp
 * @author  Will Brandon (brandon.w@northeastern.edu)
 * @date    May 1, 2023
 * 
 * @brief   Header for an abstract class that represents a graphical user interface window that is
 *          managed by multiple threads. This results in a need for correspondence between threads,
 *          often a window driver thread and a graphics thread. This interaction occurs in the form
 *          of polling events that need updating and using flags to indicate necessary actions such
 *          as closing. A window is immediately alive (open) upon its object's construction,
 *          however, it may be closed before its object's destruction.
 * 
 * @copyright Copyright (c) 2023
 */

#ifndef NONATOMIC_WINDOW_A_H_HEADER_GUARD
#define NONATOMIC_WINDOW_A_H_HEADER_GUARD
...
{
    /**
     * @brief   Represents a graphical user interface window that is managed by multiple threads.
     *          This results in a need for correspondence between threads, often a window driver
     *          thread and a graphics thread. This interaction occurs in the form of polling events
     *          that need updating and using flags to indicate necessary actions such as closing. A
     *          window is immediately alive (open) upon its object's construction, however, it may
     *          be closed before its object's destruction.
     */
    class nonatomic_window_a : public nonatomic_window_i
    {
        private:
            /**
             * @brief   Denotes whether the window is alive (open) or dead (closed) with the values
